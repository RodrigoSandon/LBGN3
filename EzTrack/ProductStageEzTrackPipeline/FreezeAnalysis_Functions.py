...
def Measure_Motion (video_dict,mt_cutoff,SIGMA=1):
    ...
    print("length of Motion:", len(Motion))
    ...
    num_frames_not_detected = 0
    print("range with len(Motion)", len(range(1,len(Motion))))
    for x in tqdm(range(1,len(Motion))):
        #print(x)
        frame_old = frame_new
        ret, frame_new = cap.read()
        if ret == True:
            ...
            num_frames_not_detected += 1
            #if no frame is detected
            x = x-1 #Reset x to last frame detected
            Motion = Motion[:x] #Amend length of motion vector
            break
    
    print("no. frames detected:", num_frames_detected)
    print("no. frames not detected:", num_frames_not_detected)

    cap.release() #release video
    time.sleep(.2) #allow printing
    print('total frames processed: {f}\n'.format(f=len(Motion)))
    return(Motion), len(Motion) #return motion values and no. motion values
...
def PlayVideo(video_dict,display_dict,Freezing,mt_cutoff,SIGMA=1):
    ...
    print("video_dict['start'] + display_dict['start']: ", video_dict['start'] + display_dict['start'])
...
########################################################################################
def calibrate_custom(video_dict,cal_pix,SIGMA,accept_p_frames=False):
    #Upoad file
    cap = cv2.VideoCapture(video_dict['fpath'])
    ...
    return cal_dif_avg, percentile, mt_cutoff
