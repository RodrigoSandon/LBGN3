...
ROOT_DIR = Path(r"/media/rory/OB 1p data/AS-GRIN_MICE")
# ^Change this path to where your videos are stored
dir_for_processed_vids = Path(r"/media/rory/OB 1p data/Processed_AS-GRIN")
...
def generate_output_dir(input_video_path: Path) -> Path:
    ...
    # OLENA's: /media/rory/OB 1p data/AS-GRIN_MICE/AS-GRIN-8/AS-GRIN-8_ext2/2021-10-21-13-59-50_video_trig_0.isxd
    ...
    ###################### Change this to alter the location of the preprocessed files ############################
    outdir = Path(f"{dir_for_processed_vids}/{mouse_name}/{session_type}")
