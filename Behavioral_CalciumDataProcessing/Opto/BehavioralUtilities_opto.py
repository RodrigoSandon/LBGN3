...
class BehavioralUtilities:
    def process_csv(df):
        ...
        return pd.Series(
            {
                "Block": block_num,
                "Trial_Type": force_or_free,
                "Reward_Size": rew_size,
                "Trial_Possible_(s)": trial_possible,
                "Start_Time_(s)": start_time,
                "Choice_Time_(s)": choice_time,
                "Collection_Time_(s)": collection_time,
                "Shock_Ocurred": shocked,
                "Omission": omission,
    ...
    def add_winstay_loseshift_loseomit(df):
        ...
        for row_idx in range(1, len(df)):  # skip the first row, always will be nan
            if (
                ...
                learning_strats.append("Win_Stay")
            elif (
                df.iloc[row_idx - 1][9] == "Loss"
                and df.iloc[row_idx][2] == "Small"
                and df.iloc[row_idx][1] == "Free"
            ):  # lose shift
                learning_strats.append("Lose_Shift")
            elif (
                df.iloc[row_idx - 1][9] == "Loss"
    ...
    def verify_table(df):
        for row_idx in range(1, len(df)):  # skip trial_num 0
            if (
                df.iloc[row_idx]["Reward_Size"] == np.nan
                and df.iloc[row_idx]["Choice_Time"] == np.nan
                and df.iloc[row_idx]["Collection_Time"] == np.nan
