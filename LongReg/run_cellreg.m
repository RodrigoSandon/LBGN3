...
% it tries to find similarities in all of them so best compare only similar
% looking sessions
% sessions = ["RM D1" "Pre-RDT RM" "RDT D1" "RDT D2" "RDT D3"];
sessions = ["Pre-RDT RM" "RDT D1"];

% Omitted: 
% "BLA-Insc-1" - done,pre-redt rm, rdt d1, rdt d2
% "BLA-Insc-2" - bc no sessions w footprints
%  "BLA-Insc-3" - bc no cell matches (prerdtrm vs rdtd2)
% "BLA-Insc-5" - rm d1, rdtd2, rdtd3 - barely no cells and they look weird
% - too different in b/w sessions
% BLA6 - non rigid transformation?prerdtrm, rdt d1, rdt d2
...
for i=1: length(mice)
    session_footprint_dirs = [];
    session_names = [];
    mouse_dir = append(rootdir, "/", mice(i));
    %plot_title = append(mice(i),"_", sessions(j));
    disp(mice(i))
    
    if not(isequal(length(sessions),0))
        for j=1: length(sessions)
            footprint_dir = append(rootdir, "/",mice(i),"/", sessions(j));

            % check to see if session has any footprints
            footprint_dir_num_files = length(dir(fullfile(footprint_dir, '/*.tif')));

            if isequal(exist(footprint_dir, "dir"), 7) && not(isequal(footprint_dir_num_files, 0))

                disp([mice(i) "and" sessions(j)]);
                footprints = dir(fullfile(footprint_dir, '*_footprint.tif'));

                % Add this current session into the larger scope list for
                % cellreg
                session_footprint_dirs = [session_footprint_dirs footprint_dir];
                session_names = [session_names append(mice(i),"_",sessions(j))];
                ...
                fname_out = fullfile(footprints(1).folder, append('spatial_footprints_', mice(i),'_', sessions(j), '.mat'));
        ...
        results_directory=fullfile(mouse_dir, 'Results') ;
        ...
        number_of_sessions=length(session_footprint_dirs);
        file_names=cell(1,number_of_sessions);
        for file_index=1:number_of_sessions
            file_names{file_index}=fullfile(session_footprint_dirs(file_index), append('spatial_footprints_',session_names(file_index), '.mat'));
        end
