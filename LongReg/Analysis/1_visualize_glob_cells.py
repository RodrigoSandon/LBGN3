    ...
    for glob_cell, local_cells_d in info_d.items():
        ...
        for idx, subevent in enumerate(range(0,1)):
            ...
            for local_cell, stats_d in info_d[glob_cell].items():
                ...
                # plot indv traces
                for trial_name, trial_trace in glob_cells_d[glob_cell][local_cell].items():
                    if sessions[0] in local_cell: 
                        ax_1.plot(t, trial_trace, color = "#9DD6F7")
                    if sessions[1] in local_cell:   
                        ax_2.plot(t, trial_trace, color = "#F79D9D")
            
                # plotting avg trace
                for stat, vals in info_d[glob_cell][local_cell].items():
                    if stat == "avg":
                        if sessions[0] in local_cell:
                            local_cell_1 = local_cell   
                            ax_1.plot(t, vals, color = "black",label=label_name)
                        if sessions[1] in local_cell:   
                            local_cell_2 = local_cell  
                            ax_2.plot(t, vals, color = "black", label=label_name)
                    elif stat == "sem":
                        difference_1 = [info_d[glob_cell][local_cell]["avg"][idx] - info_d[glob_cell][local_cell]["sem"][idx] for idx,i in enumerate(info_d[glob_cell][local_cell]["avg"])]
                        difference_2 = [info_d[glob_cell][local_cell]["avg"][idx] + info_d[glob_cell][local_cell]["sem"][idx] for idx,i in enumerate(info_d[glob_cell][local_cell]["avg"])]
                        if sessions[0] in local_cell:   
                            ax_1.fill_between(t, difference_1, difference_2, facecolor="blue")
                        if sessions[1] in local_cell:
                            ax_2.fill_between(t, difference_1, difference_2, facecolor="indianred")
            
            # get corr coef
            result = stats.pearsonr(info_d[glob_cell][local_cell_1]["avg"], info_d[glob_cell][local_cell_2]["avg"])
        ...
        fig.savefig(f"/media/rory/Padlock_DT/BLA_Analysis/LongReg/Results/Glob_Cells/{glob_cell_count}_glob_cell_{glob_cell}_{event}_{subevent_label}_{sessions[0]}_{sessions[1]}.png")
        plt.close(fig)
