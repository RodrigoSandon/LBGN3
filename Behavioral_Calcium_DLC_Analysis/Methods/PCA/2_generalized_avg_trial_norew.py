...
def main():
    ...
    parent_dir = f"Unnorm_Arranged_Dataset_{starting_timepoint}_{ending_timepoint}"
    dist_dir = f"Unnorm_Generalized_PCA_{starting_timepoint}_{ending_timepoint}"
    out_filename = "all_cells_avg_trials.csv"
    ...
    for block in blocks:
        for s in shock:
            for session in sessions:
                print(sessions)
                try:
                    root = f"/media/rory/Padlock_DT/BLA_Analysis/Decoding/{parent_dir}/{block}/{s}"
                    mice_files = find_paths_endswith(root, f"{session}/trials_average.csv")
                    print(mice_files)

                    dst = f"/media/rory/Padlock_DT/BLA_Analysis/Decoding/{dist_dir}/{block}/{s}/{session}"
                    ...
                    df_sorted.to_csv(os.path.join(dst, out_filename))
