        ...
        for combo in between_mice_d[session_type]:
            for subcombo in between_mice_d[session_type][combo]:
                ...
                concatenated_cells_df.to_csv(
                    os.path.join(new_path, "all_concat_cells_z_fullwindow.csv"), index=False
                )
...
def main():
    ...
    for i in list_of_sessions:
        print(i)
        lst_of_avg_cell_csv_paths = find_paths(ROOT_PATH, i, "concat_cells_z_fullwindow.csv") #CUSTOMIZE FOR SPECIFIC GROUPINGS YOU WANT TO PROCESS
